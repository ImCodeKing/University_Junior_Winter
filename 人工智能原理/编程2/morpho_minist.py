...
# 计算精确率
precision = precision_score(test_number, test_number_predictions, average='micro')

# 计算召回率
recall = recall_score(test_number, test_number_predictions)

# 计算F1-score
f1 = f1_score(test_number, test_number_predictions)

# 计算auROC
au_roc = roc_auc_score(test_number, test_number_predictions)

# 计算 ROC 曲线
fpr, tpr, thresholds = roc_curve(test_number, test_number_predictions)
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('2-Receiver Operating Characteristic (ROC)')
plt.legend(loc="lower right")
plt.show()
